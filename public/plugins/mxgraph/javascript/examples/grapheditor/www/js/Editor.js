...
function Dialog(editorUi, elt, w, h, modal, closable, onClose, noScroll, transparent)
{
	...
	var dh = (document.documentElement.clientHeight > 0) ? document.documentElement.clientHeight :
		Math.max(document.getElementById('graphEditorArea').clientHeight || 0, document.documentElement.clientHeight);
	var left = Math.max(1, Math.round((document.getElementById('graphEditorArea').clientWidth - w - 64) / 2));
	...
	w = Math.min(w, document.getElementById('graphEditorArea').scrollWidth - 64);
	h = Math.min(h, dh - 64);
	...
	if (modal)
	{
		document.getElementById('graphEditorArea').appendChild(this.bg);
	}
	...
	{
		dh = Math.max(document.getElementById('graphEditorArea').clientHeight, document.documentElement.clientHeight);
		this.bg.style.height = dh + 'px';

		left = Math.max(1, Math.round((document.getElementById('graphEditorArea').clientWidth - w - 64) / 2));
		top = Math.max(1, Math.round((dh - h - editorUi.footerHeight) / 3));
		w = Math.min(w0, document.getElementById('graphEditorArea').scrollWidth - 64);
		h = Math.min(h0, dh - 64);
