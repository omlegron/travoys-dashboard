...
var mxUtils =
{
	 ...
	 * 
	 * (code)
	 * document.getElementById('graphEditorArea').appendChild(mxUtils.button('Test', function(evt)
	 * {
	 *   alert('Hello, World!');
	 * }));
	 * (end)
	...
	fit: function(node)
	{
		...
		var b = document.getElementById('graphEditorArea');
	...
	getDocumentScrollOrigin: function(doc)
	{
		...
		else
		{
			...
			var x = (wnd != null && window.pageXOffset !== undefined) ? window.pageXOffset : (document.documentElement || document.getElementById('graphEditorArea').parentNode || document.getElementById('graphEditorArea')).scrollLeft;
			var y = (wnd != null && window.pageYOffset !== undefined) ? window.pageYOffset : (document.documentElement || document.getElementById('graphEditorArea').parentNode || document.getElementById('graphEditorArea')).scrollTop;
	...
	getSizeForString: function(text, fontSize, fontFamily, textWidth)
	{
		...
		div.innerHTML = text;
		document.getElementById('graphEditorArea').appendChild(div);
		
		// Gets the size and removes from DOM
		var size = new mxRectangle(0, 0, div.offsetWidth, div.offsetHeight);
		document.getElementById('graphEditorArea').removeChild(div);
	...
	popup: function(content, isInternalWindow)
	{
	   	if (isInternalWindow)
	   	{
			...
			var w = document.getElementById('graphEditorArea').clientWidth;
			var h = Math.max(document.getElementById('graphEditorArea').clientHeight || 0, document.documentElement.clientHeight)
		...
		else
		{
			...
			else
			{
			    ...
			    pre.innerHTML = mxUtils.htmlEntities(content, false).
			    	replace(/\n/g,'<br>').replace(/ /g, '&nbsp;');
			   	wnd.document.getElementById('graphEditorArea').appendChild(pre);
			}
	   	}
	},
	...
	error: function(message, width, close, icon)
	{
		...
		var w = document.getElementById('graphEditorArea').clientWidth;
		var h = (document.getElementById('graphEditorArea').clientHeight || document.documentElement.clientHeight);
	 ...
	 * document.getElementById('graphEditorArea').appendChild(img);
	 * (end)
	 * 
	 * Parameters:
	 * 
